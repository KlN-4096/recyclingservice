...
- [x] 修改区块卸载方式,改为区块冻结,要求冻结后可以解冻
- [x] 有个小问题,现在的删除方式与放入垃圾箱会有时间延迟,即放入了垃圾箱但地上的物品还没被清理
- [ ] 物品数量显示实际数量(通过已有的背包模组进行显示,如果没有相关模组,还是tooltip显示)
- [ ] 可以复用区块搜索常加载区块的逻辑,新增一个找这种大片常加载区块的命令
- [ ] 上报时间阈值可以配置
- [ ] 如果有玩家ticket就记录时间，当当前时间和记录时间超过2天后，视为无玩家区块，永久卸载直到重新有玩家ticket
- [ ] 接管区块会接管大量不知来源的区块,检查并修复
- [x] 平时改为10s扫描一次,临近清理时改为1s,然后全局信号超时时间改回5s
- [ ] 删除ErrorHelper,有点过度包装了
- [ ] 接管区块可以试试mixin避免区块全量扫描
- [ ] 让Manager完全代理各个模块中方法,元素的对外使用
- [ ] 解冻也应该是范围解冻,避免机器破坏
- [ ] 低于xx方块实体数量冻结可以取消了,性能损失不大,反而会影响其它